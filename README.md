...
예약 → 기다림 → 수신 → 대화 → 기록을 체험하는 모바일 앱입니다. 따뜻한 필름 사진, 세로 결 유리, 크림색 반투명 패널과 올리브색 버튼을 사용합니다.
...
계정이 준비되지 않아 실제 Twilio/OpenAI/Supabase 연동은 외부 서비스에서 검증하지 않았습니다. 영상통화·실제 아티스트 통화·결제·목소리 복제는 구현 범위에 포함하지 않았습니다. 영상통화는 향후 확장 인터페이스만 준비했습니다.
